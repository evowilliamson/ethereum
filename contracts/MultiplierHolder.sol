...
contract MultiplierHolder is Owned, MultiplierHolderI  {
    ...
    mapping(uint => uint) vehicleTypeToMultipliers;
    ...
    function setMultiplier(uint vehicleType, uint multiplier) 
        ...
        require(vehicleType > 0, "Vehicle type must be greater than 0");
        require(vehicleTypeToMultipliers[vehicleType] != multiplier, 
                "New multiplier must be different for vehicle type");

        vehicleTypeToMultipliers[vehicleType] = multiplier;
     ...
     */
    function getMultiplier(uint vehicleType) view public returns (uint) {

        return vehicleTypeToMultipliers[vehicleType];
